...
- [Глобальная балансировка](#глобальная-балансировка)
    - [Расчет распределение запросов из секции "Расчет нагрузки" по типам запросов по датацентрам](#расчет-распределение-запросов-по-датацентрам)
    - [Описание способов балансировки](#описание-способов-балансировки)
    - [Как работает балансировка](#как-работает-балансировка)
...
Взглянем на карту плотности населения Росиии (на ней обозначены местоположения ЦОД, CDN и DNS):
...
Также расположим DNS сервера Yandex, которые будут маршрутизировать клиентов к серверам сервиса. На сервера DNS не оказывается большая нагрузка,
поэтому можно расположить их побольше, чтобы уменьшить latency первичных запросов.

### Расчет распределение запросов по датацентрам
Точных данных нет, но можно предположить, что нагрузка на ЦОДы будет делиться в таком соотношении (учитывая географическое расположение): `5:3:3:2`, 
начиная с Москвы и заканчивая Красноярском.

| Расположение датацентра   | RPS    | Пиковый RPS | Нагрузка на сеть GB/sec | Пиковая нагрузка на сеть GB/sec |
|---------------------------|--------|-------------|-------------------------|---------------------------------|
| Москва                    | 3431.4 | 8578.5      | 7.338                   | 18.345                          |
| Санкт-Петербург           | 2058.9 | 5147.25     | 4.403                   | 11.007                          |
| Краснодар                 | 2058.9 | 5147.25     | 4.403                   | 11.007                          |
| Красноярск                | 1372.6 | 3431.5      | 2.935                   | 7.338                           | 

### Описание способов балансировки
Для балансировки нагрузки на сервера будем использовать GeoDNS и latency-based DNS балансировки. Последняя из упомянутых основана на значении latensy сервера, что позволяет делать запрос к менее загруженному серверу и не перегружать более загруженный.

### Как работает балансировка
Пример запроса к серверу для получения главной страницы:
+ Пользователь вбивает в браузер music.yandex.ru/;
+ Запрос меняется на music.yandex.ru/home;
+ Локальный балансировщик ищет закэшированное значение ip-адреса целевого ресурса;
  + Если находит ip-адрес, то делает запрос к серверу и получает статику и медиа контент через асинхронные запросы.
+ Локальный балансировщик не находит значение ip-адреса целевого ресурса, делает запрос к tier-1/tier-2 провайдеру, чтобы проверить, есть ли у него закэшированная запись ip-адреса. 
  + Если находит, то отдает пользователю и дальше делается запрос к серверу Yandex Music.
+ Провайдер не находит у себя записи адреса и обращается к корневому DNS-серверу `.`, который возвращает ip-адреса DNS-серверов, отвечающих за домен верхнего уровня (TLD) `ru`. Возращается список, сбалансированный по принципу, нам не известному.
+ Провайдер снова делает запрос, но уже к DNS серверам Yandex зоны `yandex.ru`. Возвращается список адресов DNS-серверов `music.yandex.ru` на основе GeoDNS (провайдер передает название сети клиента).
+ Провайдер делает запрос к ближайшему DNS серверу, получает список серверов на основе latency. 
+ Провайдер возвращает список серверов, где на первом месте сервер с самым маленьким значением latency для клиента.
+ Клиент делает запрос, получает статику и кэширует адрес.
