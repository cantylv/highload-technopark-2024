...
| Действие                                | Запрос/день на одного исполнителя    |
|-----------------------------------------|--------------------------------------|
...
+ Открытие списка избранных песен (в среднем `5 раз/день`): 
  + Подгрузка части списка (infinite scroll) - 20 названий песни и столько же картинок по `10KB`, в среднем избранных песен 100.
+ Открытие плейлиста (в среднем `2 раз/день`):
  + Подгрузка части списка (infinite scroll) - 20 названий песни и столько же картинок по `10KB`, в среднем песен плейлиста 40.
+ Открытие альбома (в среднем `2 раз/день`):
  + Подгрузка части списка (infinite scroll) - 15 названий песни и столько же картинок по `10KB`, в среднем песен альбома 40.
...
+ Получение истории прослушиваний (в среднем `1 раз/день`): 
  + Подгрузка части списка (infinite scroll) - 15 названий песни и столько же картинок по `10KB`. История прослушиваний приходит за 2 недели, поэтому количество треков в среднем равно `10 * 14 = 140`. 
...
+ Загрузка трека исполнителем (в среднем `7860 раз/день`):
...
|-----------------------------------------|--------------------------------------|
| Авторизация                             | 3.7                                  |
| Донаты исполнителям                     | 0.3                                  |
| Создание коллекций треков               | 3.7                                  |
| Получение главной страницы              | 347                                  |
| Получение списка избранных песен        | 579                                  |
| Получение плейлиста                     | 232                                  |
| Получение альбома                       | 232                                  |
| Поиск треков                            | 347                                  |
| Поиск альбомов                          | 347                                  |
| Поиск исполнителей                      | 347                                  |
| Поиск плейлистов                        | 347                                  |
| Поиск подкастов                         | 347                                  |
| Поиск аудиокниг по названию             | 347                                  |
| Стриминг избранного трека               | 1157                                 |
| Получение истории прослушиваний         | 116                                  |
| Добавление песни в избранное            | 232                                  |
| Прослушивание рекомендации 'Моя волна'  | 1157                                 |
| **Суммарно**                            | **6042**                             |
...
$$
  handlerRPS = \frac{5 \times 10 \times 10^6}{86400} = 579 RPS
$$
