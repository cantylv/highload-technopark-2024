...
+ memoryPerUser - размер памяти, занимаемый одним пользователем.  
...
**Пояснение:** Связанные данные включают в себя метаданные о ресурсе (формат, расширение, алгорит кодирования и тп).
...
где:
+ audioFileSize - размер памяти, занимаемый треком;
...
$$
  audioFileSize = \frac{3 * 60 * 320kb}{8 * 1024} = 6.86MB
$$
...
+ audioFileSize - размер памяти, занимаемый одним треком.  
...
Аналогичный расчет, средняя продолжительность эпизода подкаста равняется `30 минутам`[^10]. Проводим рассчеты:
